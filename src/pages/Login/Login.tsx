...
import { service_login, service_getUserInfo } from '../../service/account_service';
    ...
    const onFinish = async (values: any) => {
        ...
        try {
            ...
            if (rsp?.status === 200) {
                ...
                const { hospital, aid: doctorId, account_name: accountName } = userInfo.data.data.account;
                ...
                aliCloudLog(`Login页面 用户名或密码错误 rsp: ${JSON.stringify(rsp)}` + username);
            }
        } catch (error) {
            aliCloudLog(`Login页面 登录失败 error: ${JSON.stringify(error)}` + username);
        }
    };
            ...
            <Card className="card-login">
                ...
                >
                    <Form.Item<FieldType>
                        ...
                        layout="horizontal"
                    >
                        <Input prefix={<UserOutlined />} size="large" placeholder="请输入用户名" style={{ width: '100%' }} />
                    </Form.Item>

                    <Form.Item<FieldType> label="" name="password" rules={[{ required: true, message: '请输入密码' }]}>
                        <Input.Password
                            size="large"
                            prefix={<LockOutlined />}
                            placeholder="请输入密码"
                    ...
                    <Form.Item<FieldType> name="rememberMe" wrapperCol={{ span: 24 }} valuePropName="checked">
                        <Checkbox style={{ width: '100%' }}>记住我</Checkbox>
                    </Form.Item>

                    <Form.Item wrapperCol={{ span: 24 }}>
                        <Button type="primary" className="btn-login" size="large" htmlType="submit">
                            登录
                        </Button>
                    </Form.Item>
                </Form>
